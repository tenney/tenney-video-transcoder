...
public class DefaultFFMPEGLocator extends FFMPEGLocator {
    ...
    private Logger logger = Logger.getLogger(DefaultFFMPEGLocator.class);
	/**
	...
	public DefaultFFMPEGLocator() {
		...
		logger.info("初始化转码程序:" + exe.getAbsolutePath());
		if (!exe.exists() || exe.length() < 1) {
		    String ffmpegPath = "/linux";
		    if(isMac) ffmpegPath = "/mac";
		    if(isWindows) ffmpegPath = "/windows";
		    
			copyFile(ffmpegPath + "/ffmpeg" + suffix, exe);
		}
		// pthreadGC2.dll
		if (isWindows) {
			File dll = new File(temp, "pthreadGC2.dll");
			if (!dll.exists() || dll.length() < 1) {
				copyFile("/windows/pthreadGC2.dll", dll);
			}
		}
		// Need a chmod?
		if (!isWindows) {
	...
	private void copyFile(String path, File dest) throws RuntimeException {
		...
		try {
//		    System.out.println(DefaultFFMPEGLocator.class.getResource(path).getFile());
